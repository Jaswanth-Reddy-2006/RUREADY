...
export default function CodingAnalyticsSection({ codingBreakdown }: CodingAnalyticsSectionProps) {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-[#DCE7F2] pb-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-[#526078]">
              Problem understanding, approach, code correctness, complexity, debugging & testing.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs font-mono font-bold px-3 py-1 rounded-xl bg-slate-100 border border-slate-200 text-slate-700">
            {codingBreakdown.totalSessions} Coding Session{codingBreakdown.totalSessions === 1 ? '' : 's'}
          </span>
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-[#DCE7F2]">
          <h4 className="text-xs font-bold uppercase tracking-wider text-[#526078]">
            Coding Evaluation Breakdown
          </h4>
          <div className="space-y-2 text-xs">
            {Object.entries(codingBreakdown.metrics).map(([label, val]) => (
        ...
        {/* Right: Recent Coding Sessions */}
        <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-[#DCE7F2]">
          <h4 className="text-xs font-bold uppercase tracking-wider text-[#526078]">
            Recent Coding Sessions
          </h4>

          {codingBreakdown.recentSessions.length > 0 ? (
            <div className="space-y-2">
              {codingBreakdown.recentSessions.slice(0, 3).map((sess) => (
                <div
                  key={sess.id}
                  onClick={() => navigate(sess.analysisUrl)}
                  className="p-3 bg-white hover:bg-purple-50 border border-slate-200 rounded-xl flex items-center justify-between gap-3 text-xs cursor-pointer transition-colors group"
                >
                  <div>
                    <p className="font-bold text-[#11183D] group-hover:text-purple-700 transition-colors">{sess.title}</p>
                    <p className="text-[11px] text-[#526078] font-mono">{sess.date}</p>
                  </div>

                  <div className="flex items-center gap-2">
                    <span className="font-mono font-bold text-sm text-purple-700">{sess.score}%</span>
                    <ExternalLink size={13} className="text-slate-400 group-hover:text-purple-700" />
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-xs text-[#526078] italic">No recent coding sessions recorded.</p>
          )}
        </div>
      </div>
    </div>
