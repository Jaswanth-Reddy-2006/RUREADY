...
export interface SparkleNavbarProps {
  /**
  ...
  color?: string;
  /**
   * Optional controlled active item index.
   */
  activeIndex?: number;
  /**
...
/**
 * SparkleNavbar from Lightswind with GSAP SVG beam & lightning strike animations.
 */
const SparkleNavbar: React.FC<SparkleNavbarProps> = ({
  ...
  const getOffsetLeft = (element: HTMLButtonElement) => {
    ...
    const activeElementWidth = activeElementRef.current?.offsetWidth || 28;
    return (
      elementRect.left -
      navRect.left +
  ...
  // Animate whenever activeIndex changes (either via click or scroll)
  const animateTo = (newIndex: number) => {
    const navElement = navRef.current;
    const activeElement = activeElementRef.current;
    const oldIndex = prevIndexRef.current;
    const oldButton = buttonRefs.current[oldIndex];
    const newButton = buttonRefs.current[newIndex];

    if (!navElement || !activeElement || !oldButton || !newButton || newIndex === oldIndex) {
      return;
    }
    ...
    gsap.to(activeElement, {
      keyframes: [
        {
          "--active-element-width": `${spacing > navElement.offsetWidth - 30 ? navElement.offsetWidth - 30 : spacing}px`,
          duration: 0.3,
          ease: "none",
          onStart: () => {
  ...
  // Sync with controlled index updates
  useEffect(() => {
    if (controlledIndex !== undefined && controlledIndex !== prevIndexRef.current) {
      animateTo(controlledIndex);
    }
  }, [controlledIndex]);
  ...
  return (
    <div className={`sparkle-navbar-wrapper relative inline-flex items-center ${className}`}>
      <style>{`
        .navigation-menu {
          margin: 0;
          position: relative;
        ...
        .navigation-menu ul li:not(.active):hover button {
          text-shadow: 0 0 8px ${color}80, 0 0 16px ${color}40;
        }
            ...
            return (
              <li key={item} className={isActive ? "active" : ""}>
                <button
                  ...
                  className={`text-xs sm:text-sm font-sans transition-all select-none ${
                    isActive
                      ? "text-[#11183D] font-extrabold"
                      : "text-[#526078] font-semibold hover:text-[#11183D]"
                  }`}
                >
                  {item}
                </button>
