...
export default function Dashboard() {
  ...
  if (isLoading) {
    return (
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 space-y-8">
        <div className="h-40 bg-white border border-slate-200 rounded-3xl animate-pulse" />
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 space-y-8 text-slate-800">
      ...
      <motion.div
        ...
        className="relative overflow-hidden rounded-3xl bg-white p-8 sm:p-10 text-slate-900 shadow-sm border border-slate-200"
      >
        ...
        <div className="absolute bottom-0 right-1/4 w-60 h-60 bg-blue-100/40 rounded-full blur-[80px] pointer-events-none" />

        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          <div className="max-w-2xl space-y-3">
            <div className="inline-flex items-center gap-2 rounded-full bg-orange-50 border border-orange-200/80 px-3.5 py-1 text-[#FF7A00]">
            ...
            {profileData.targetRole && (
              <div className="flex flex-wrap items-center gap-2 pt-1 text-xs text-slate-500">
                <span className="flex items-center gap-1 font-semibold text-slate-700">
                  <Briefcase size={13} className="text-[#FF7A00]" />
                  {profileData.targetRole}
                </span>
                {profileData.targetCompany && (
                  <>
                    <span>•</span>
                    <span className="flex items-center gap-1 font-semibold text-slate-700">
                      <Building2 size={13} className="text-[#FF7A00]" />
                      {profileData.targetCompany}
                    </span>
                  </>
                )}
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            ...
            <Link to="/interview/coding/new">
              <Button variant="secondary" size="lg" icon={<Code2 className="h-4 w-4 text-[#FF7A00]" />}>
                Start Coding Track
              </Button>
            </Link>
          </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-500 font-body">Objective Socratic Engine</span>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {[
            ...
            >
              <div className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col justify-between h-full hover:border-orange-300 hover:shadow-sm transition-all">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 font-body mt-1 truncate">
                    {metric.desc}
                  </p>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-2.5 w-full sm:w-auto">
                <div className="relative flex-1 sm:w-48">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-[#FF7A00] focus:ring-2 focus:ring-orange-500/20 font-body transition-all"
                  />
                </div>

                <div className="flex rounded-xl bg-slate-100 p-0.5 border border-slate-200 text-xs font-display">
                  {['ALL', 'COMPLETED', 'IN_PROGRESS'].map(type => (
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg font-semibold transition-all cursor-pointer ${
                        filterType === type 
                          ? 'bg-white text-[#FF7A00] shadow-sm font-bold' 
            ...
            {filteredSessions.length === 0 ? (
              <div className="text-center py-16 space-y-4">
                <div className="h-14 w-14 rounded-2xl bg-orange-50 border border-orange-200 flex items-center justify-center text-[#FF7A00] mx-auto shadow-sm">
              ...
              <div className="divide-y divide-slate-100">
                {filteredSessions.map((session) => {
                  ...
                  return (
                    <div
                      key={session.id}
                      className="py-4.5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50/80 -mx-4 px-4 transition-colors group rounded-xl"
                    >
                      {/* Left info */}
                      <div className="space-y-1 min-w-0">
                        <div className="flex items-center gap-2.5 flex-wrap">
                          <span className="font-display font-bold text-slate-800 text-base group-hover:text-[#FF7A00] transition-colors truncate">
                            {session.targetRole}
                          </span>
                        ...
                        <div className="flex items-center gap-3 text-xs text-slate-500 font-body">
                          ...
                          {session.targetCompany && (
                            <>
                              <span>•</span>
                              <span className="text-slate-700 font-medium">{session.targetCompany}</span>
                            </>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4 self-end sm:self-center shrink-0">
                        {isAnalyzed ? (
                          ...
                          <span className="text-xs text-slate-500 font-body font-medium">In Progress</span>
                        )}

                        {isAnalyzed ? (
                          <Link to={`/analysis/${session.id}`}>
                            <Button variant="secondary" size="sm" iconRight={<ChevronRight className="h-3.5 w-3.5 text-[#FF7A00]" />}>
                              Report
                            </Button>
                          </Link>
                        ) : (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-orange-50/80 via-white to-white border border-orange-200 p-6 sm:p-7 rounded-3xl shadow-sm relative overflow-hidden">
            <div className="flex items-center gap-2 mb-3">
              <Sparkles className="h-4 w-4 text-[#FF7A00]" />
              <span className="text-xs font-bold uppercase tracking-wider text-[#FF7A00] font-display">
            ...
            <p className="text-xs text-slate-600 font-body leading-relaxed mb-5">
              Reinforce situation, task, action, and result structuring with live real-time follow-up probes.
            </p>

            <Link to="/interview/new">
              <Button size="md" fullWidth iconRight={<ArrowRight className="h-4 w-4" />}>
          ...
          <div className="bg-white border border-slate-200 p-6 sm:p-7 rounded-3xl shadow-sm">
            <h3 className="font-display font-bold text-xs text-slate-500 uppercase tracking-wider mb-4 flex items-center gap-1.5">
            ...
            <div className="space-y-3">
              {[
                ...
                <Link
                  key={i}
                  to={track.href}
                  className="flex items-center justify-between p-3.5 rounded-xl border border-slate-100 bg-slate-50/60 hover:border-orange-200 hover:bg-orange-50/40 transition-all group"
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="p-2 rounded-lg bg-white border border-slate-200 text-slate-600 group-hover:text-[#FF7A00] group-hover:border-orange-300 transition-colors shadow-xs">
                      <track.icon className="h-4 w-4" />
                    </div>
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-slate-800 group-hover:text-[#FF7A00] transition-colors truncate font-display">
                        {track.title}
                      </p>
                  ...
                  <ChevronRight className="h-4 w-4 text-slate-400 group-hover:text-[#FF7A00] group-hover:translate-x-0.5 transition-all shrink-0" />
                </Link>
              ))}
            </div>
          </div>
