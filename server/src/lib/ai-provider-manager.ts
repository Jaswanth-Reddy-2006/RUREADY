...
class AIProviderManager {
  ...
  /**
   * Health ping to verify provider availability and latency.
   */
  public async testConnection(): Promise<{ success: boolean; latencyMs: number; message: string; discoveredModels?: string[] }> {
    ...
    try {
      ...
      // If Ollama, try /api/tags or /models endpoint first to discover models
      ...
      // Test standard chat completion ping
      ...
      return {
        success: false,
        latencyMs: Date.now() - start,
        message: `Connection failed: ${err.message || 'Unknown network error'}. Make sure local server is running.`,
      };
    }
  }

  /**
   * Execute chat completion via the currently configured provider.
   */
