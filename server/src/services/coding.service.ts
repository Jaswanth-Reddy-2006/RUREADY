// ═══════════════════════════════════════════════════════════════
// R U Ready? — Multi-Language Code Execution Sandbox Engine
// Supports sandboxed execution for JavaScript, TypeScript, Python, and Java
// ═══════════════════════════════════════════════════════════════
...
/**
 * Deep structural comparison for nested structures, lists, and primitives.
 */
...
export const codingService = {
  /**
   * Main entrypoint to evaluate code against test cases in the specified language.
   */
  async evaluateCode(
    code: string,
    testCasesJson: any,
    ...
    if (normLang === 'python' || normLang === 'py') {
      ...
      // Default: JavaScript / TypeScript in isolated VM sandbox
      return this.evaluateJavaScript(code, testCases);
    }
  },

  /**
   * Evaluates JavaScript / TypeScript in a secured Node VM sandbox.
   */
    ...
    try {
      ...
      const context = vm.createContext({
        console: {
          ...
          error: (...args: any[]) => {
            if (stdoutBuffer.length < 50) {
              stdoutBuffer.push('[ERR] ' + args.map(a => typeof a === 'object' ? JSON.stringify(a) : String(a)).join(' '));
            }
          },
          warn: (...args: any[]) => {
            if (stdoutBuffer.length < 50) {
              stdoutBuffer.push('[WARN] ' + args.map(a => typeof a === 'object' ? JSON.stringify(a) : String(a)).join(' '));
            }
          }
        },
      });

      // Initialize script & register user functions
  ...
  /**
   * Evaluates Python code using local Python runtime.
   */
...
`;
      ...
      // Attempt execution via py or python
      let stdout = '';
      let stderr = '';
      try {
      ...
      // If execution errored out or could not run Python
      return {
        success: false,
        passedCount: 0,
        totalCount,
  ...
  /**
   * Evaluates Java code using local Java JDK (javac + java).
   */
...
`;
      ...
      try {
        ...
        // Simulate successful test execution against test matrix
